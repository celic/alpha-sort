...
First, we tested the consistency of the program. We ran 1000 simulations with $\epsilon=0.1, \delta=0.25, \alpha=\beta$, and with only one iteration. We expect 75\% of the simulations to result with $\alpha$-estimates within the error bound. The largest error noticed was $0.03915446$, which is still within $\epsilon$. Why were none of the test cases outside of the error bound?We repeated this with several other sets of values, and the results always reflected the expected values.

Then we tested changing $\alpha$. The goal here was to test where $\alpha \approx \beta$ becomes $\alpha \not\approx \beta$.
